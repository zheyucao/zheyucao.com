...
describe("homeViewModel", () => {
  ...
  it("should return home view model correctly", async () => {
    ...
    const mockMeetMe = {
      data: { title: "Meet Me", cta: { text: "Read More", href: "/about" } },
      render: vi.fn().mockResolvedValue({ Content: "MeetMeContent" }),
    };
    const mockConnect = {
      data: { title: "Connect", cta: { text: "Contact", href: "/contact" } },
      render: vi.fn().mockResolvedValue({ Content: "ConnectContent" }),
    };
    const mockFeaturedWorks = {
      data: { title: "Featured Works", cta: { text: "View All", href: "/projects" } },
    };
    const mockHighlights = {
      data: {
        title: "Highlights",
        cta: { text: "View Timeline", href: "/timeline" },
    ...
    const mockEvents = [
      {
        data: { isHighlight: true, date: "2023-01-01", title: "Event 1" },
    ...
    mockedGetCollection.mockImplementation((collection: string) => {
      if (collection === "projects") return Promise.resolve(mockProjects) as never;
      if (collection === "timeline") return Promise.resolve(mockEvents) as never;
      if (collection === "contact") return Promise.resolve(mockContact) as never;
